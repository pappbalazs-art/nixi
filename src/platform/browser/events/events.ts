...
import { VirtualNode, getAttribute } from "@core/vdom";
import { isFunction } from "@helpers";
import { getDOMElementByRoute } from "../dom";
...
function makeEvents(vNode: VirtualNode, uid: number) {
	...
	if (vNode.type === "TAG") {
		...
		const children = vNode.children.filter(filterNodeFn);
...
function delegateEvent(
	...
	app.eventHandlers.get(node)[eventName] = {
		...
		removeEvent: () =>
			rootDOMElement.removeEventListener(eventName, eventHandler),
	};
